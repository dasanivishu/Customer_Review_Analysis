...
# Load the keywords DataFrame from CSV
...
def drawTimeSeriesFigure(fig=None):
    ...
    return html.Div([
        dbc.Card(dbc.CardBody([
            html.P('Review Sentiments Trend'),
            dcc.Graph(
                figure=fig.update_layout(
                    template='plotly_dark',
                    plot_bgcolor='rgba(0, 0, 0, 0)',
...
def get_recent_reviews(review_business_data_merged, id):
    ...
    data_most_reviewed_store = data_most_reviewed_store.drop(columns=['business_id', 'review_id', 'user_id', 'categories'])
...
data_most_reviewed_store['date_str'] = date_str

# Group by the 'date_str' column for sentiments and sum
data_most_reviewed_store_timeseries_pos = data_most_reviewed_store[data_most_reviewed_store['sentiments'] == 1].groupby('date_str')[['date_str', 'sentiments']].sum()
data_most_reviewed_store_timeseries_neg = data_most_reviewed_store[data_most_reviewed_store['sentiments'] == 3].groupby('date_str')[['date_str', 'sentiments']].sum()
data_most_reviewed_store_timeseries_neu = data_most_reviewed_store[data_most_reviewed_store['sentiments'] == 2].groupby('date_str')[['date_str', 'sentiments']].sum()

# Add a 'rating' column for each group
data_most_reviewed_store_timeseries_pos['rating'] = ['Positive' for _ in range(len(data_most_reviewed_store_timeseries_pos))]
data_most_reviewed_store_timeseries_neg['rating'] = ['Negative' for _ in range(len(data_most_reviewed_store_timeseries_neg))]
data_most_reviewed_store_timeseries_neu['rating'] = ['Neutral' for _ in range(len(data_most_reviewed_store_timeseries_neu))]

# Concatenate the dataframes
data_most_reviewed_store_timeseries = pd.concat([data_most_reviewed_store_timeseries_pos, data_most_reviewed_store_timeseries_neg, data_most_reviewed_store_timeseries_neu])
...
def drawText(text='NA'):
    return html.Div([
        dbc.Card(
            dbc.CardBody([
                html.Div([
                    html.P('Selected Business', style={'color': 'white'}),
...
def drawTextOverallRating(text='NA'):
    return html.Div([
        dbc.Card(
            dbc.CardBody([
                html.Div([
                    html.P('Overall Rating', style={'color': 'white'}),
                    html.H5(text, style={'color': 'yellow'}),
                ], style={'textAlign': 'center'})
            ])
        )
...
def drawTotalReview(text='Text'):
    return html.Div([
        dbc.Card(
            dbc.CardBody([
                html.Div([
                    html.P('Total Review', style={'color': 'white'}),
                    html.H5(text, style={'color': 'yellow'}),
                ], style={'textAlign': 'center'})
            ])
        )
...
def update_output_div(input_value):
    ...
    return (
        drawText(resp['business_name']),
        drawTextOverallRating(resp['business_rating']),
        drawTotalReview(resp['num_review']),
        drawRatings(resp['rating_fig']),
        drawPieFigure(resp['sentiment_fig']),
        drawRecentReviews(recent_reviews),
        drawTrendsBarFigure(resp['timeseries_bar_fig']),
        drawTrendingWords(pos_keywords, neg_keywords),
    )
